...
function initLayout() {
    ...
    if(header) header.style.width = `${boardPx}px`;
    ...
    for (let y = 0; y < gridCount; y++) {
        for (let x = 0; x < gridCount; x++) {
            if ((x + y) % 2 !== 0) {
                ...
                tile.style.backgroundColor = 'rgba(0,0,0,0.05)'; // subtle grid
...
function draw() {
    ...
    snake.forEach((part, i) => {
        ...
        partEl.style.width = partEl.style.height = `${cellSize}px`;
        
        // Use loaded image or fallback color
        if (loadedImages[i % loadedImages.length].complete) {
            partEl.style.backgroundImage = `url("${loadedImages[i % loadedImages.length].src}")`;
        } else {
            partEl.style.backgroundColor = '#3498db';
        }
